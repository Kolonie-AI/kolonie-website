...
- **Adding a picture has a written procedure; do not invent one** (#135).
  **An icon** is hand-drawn SVG on the 24×24 grid, geometry only, and its name
  goes into `ICON_NAMES` in the same commit — steps and reasoning in
  `src/icons/README.md`. **An illustration** is generated by an image model
  against a prompt that names the tokens and never a hex, and every pixel has to
  survive `npm run check:illustrations` — steps, prompt constraints and a
  good-versus-bad example in `public/illustrations/README.md`. Both sets are on
  `/visuals/`, which is the page to look at before drawing a thirteenth of
  anything; the mark itself belongs to `kolonie-docs/brand/README.md` and is not
  the website's to redraw.
